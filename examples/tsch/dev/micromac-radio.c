...
micromac_radio_start_rx_delayed(uint32 u32_delay_time, uint32 u32_on_duration)
{
	...
	vMMAC_StartMacReceive(rx_frame_buffer_write_ptr, E_MMAC_RX_DELAY_START
			| E_MMAC_RX_NO_AUTO_ACK | E_MMAC_RX_ALLOW_MALFORMED
...
//		on();
		...
		if (!rx_state) {
			...
			rx_end_time = last_packet_timestamp + rx_frame_buffer_recent_ptr->u8PayloadLength +1;
			...
			if(need_ack) {
				...
				if(softack_make_callback != NULL) {
					...
					softack_make_callback(&ackbuf, rx_frame_buffer_recent_ptr->u8SequenceNum, last_packet_timestamp, 0);
...
//				on();
		...
		if(interrupt_exit_callback != NULL) {
			...
			last_rf.sfd_timestamp = last_packet_timestamp; //u32MMAC_GetRxTime();
...
micromac_radio_send_ack(void) {
  if(!locked) {
		GET_LOCK();
		tx_in_progress = 1;
		vMMAC_StartPhyTransmit(&phy_ackbuf, E_MMAC_TX_START_NOW
...
micromac_radio_send_ack_delayed(uint32 u32_delay_time)
{
  if(!locked) {
		...
		vMMAC_StartPhyTransmit(&phy_ackbuf, E_MMAC_TX_DELAY_START
				| E_MMAC_TX_NO_AUTO_ACK | E_MMAC_TX_NO_CCA);
		tx_in_progress = 0;
		RELEASE_LOCK();
  }
...
int
micromac_radio_init_with_irq(void)
{
	if (locked) {
		return 0;
	}
...
micromac_radio_raw_transmit_delayed(tsPhyFrame *psFrame, uint32 u32_delay_time)
{
	...
	tx_in_progress = 1;
	vMMAC_SetTxStartTime(u32MMAC_GetTime() + u32_delay_time);
	vMMAC_StartPhyTransmit(psFrame, E_MMAC_TX_DELAY_START
			| E_MMAC_TX_NO_AUTO_ACK | E_MMAC_TX_NO_CCA);
	tx_in_progress = 0;
	RELEASE_LOCK();
}
...
micromac_radio_raw_transmit(tsPhyFrame *psFrame)
{
	...
	GET_LOCK();
	tx_in_progress = 1;
	vMMAC_StartPhyTransmit(psFrame, E_MMAC_TX_START_NOW
			| E_MMAC_TX_NO_AUTO_ACK | E_MMAC_TX_NO_CCA);
//	uint32 u32Mask = E_MMAC_INT_TX_COMPLETE;
	while (!u32MMAC_PollInterruptSource(E_MMAC_INT_TX_COMPLETE)){}
	tx_in_progress = 0;
